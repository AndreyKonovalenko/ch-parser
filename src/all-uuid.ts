...
for(const index in loans.LOAN)  {
  ...
  const arraers = loan.PASTDUE_ARREARS ? (loan.PASTDUE_ARREARS.PASTDUE_ARREAR ? loan.PASTDUE_ARREARS.PASTDUE_ARREAR: null) : null
  if(arraers && arraers.length > 0) {
    const result = pastdueArrearsHandler(arraers, loans.LOAN[index].UUID ? loans.LOAN[index].UUID: undefined)
